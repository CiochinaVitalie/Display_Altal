...
UXTB	R0, R4
BL	_calculateCRC+0
;modbus.c,165 :: 		if (calculated_crc == recieved_crc) // verify checksum
CMP	R0, R7
IT	NE
BNE	L_check_F3_data10
; recieved_crc end address is: 28 (R7)
;modbus.c,167 :: 		unsigned char index = 3;
; index start address is: 28 (R7)
MOVS	R7, #3
;modbus.c,168 :: 		unsigned char i = 0;
;modbus.c,169 :: 		int incAdr=0;
; incAdr start address is: 0 (R0)
MOVW	R0, #0
SXTH	R0, R0
;modbus.c,172 :: 		for (i = 0; i < no_of_registers; i++)
...
;modbus.c,175 :: 		system_reg[bus_data.address + incAdr] = (frame[index] << 8) | frame[index + 1];
...
ORR	R1, R3, R1, LSL #0
STRH	R1, [R4, #0]
;modbus.c,180 :: 		index += 2;
ADDS	R1, R7, #2
UXTB	R7, R1
;modbus.c,181 :: 		incAdr+=10;
ADDS	R6, #10
SXTH	R6, R6
;modbus.c,172 :: 		for (i = 0; i < no_of_registers; i++)
ADDS	R5, R5, #1
...
L_check_F3_data12:
;modbus.c,183 :: 		msgOk = true; // message successful
MOVS	R2, #1
...
_check_F16_data:
;modbus.c,188 :: 		void check_F16_data()
SUB	SP, SP, #4
STR	LR, [SP, #0]
;modbus.c,190 :: 		unsigned int recieved_address = ((frame[2] << 8) | frame[3]);
MOVW	R0, #lo_addr(_frame+2)
MOVT	R0, #hi_addr(_frame+2)
LDRB	R0, [R0, #0]
LSLS	R1, R0, #8
...
UXTH	R6, R0
;modbus.c,191 :: 		unsigned int recieved_registers = ((frame[4] << 8) | frame[5]);
MOVW	R0, #lo_addr(_frame+4)
MOVT	R0, #hi_addr(_frame+4)
LDRB	R0, [R0, #0]
LSLS	R1, R0, #8
...
UXTH	R7, R0
;modbus.c,192 :: 		unsigned int recieved_crc = ((frame[6] << 8) | frame[7]); // combine the crc Low & High bytes
MOVW	R0, #lo_addr(_frame+6)
MOVT	R0, #hi_addr(_frame+6)
LDRB	R0, [R0, #0]
LSLS	R1, R0, #8
...
UXTH	R8, R0
;modbus.c,193 :: 		unsigned int calculated_crc = calculateCRC(6); // only the first 6 bytes are used for crc calculation
MOVS	R0, #6
BL	_calculateCRC+0
; calculated_crc start address is: 4 (R1)
UXTH	R1, R0
;modbus.c,197 :: 		recieved_registers == bus_data.no_of_registers &&
MOVW	R0, #lo_addr(_bus_data+2)
MOVT	R0, #hi_addr(_bus_data+2)
LDRH	R0, [R0, #0]
CMP	R6, R0
...
;modbus.c,198 :: 		recieved_crc == calculated_crc){
CMP	R8, R1
IT	NE
...
L__check_F16_data30:
;modbus.c,199 :: 		messageOkFlag = 1; // message successful
MOVS	R1, #1
MOVW	R0, #lo_addr(_messageOkFlag+0)
MOVT	R0, #hi_addr(_messageOkFlag+0)
STRB	R1, [R0, #0]
;modbus.c,200 :: 		pushButton = false; }
MOVS	R1, #0
...
_sendbus_data:
;modbus.c,205 :: 		void sendbus_data(unsigned char bufferSize)
; bufferSize start address is: 0 (R0)
SUB	SP, SP, #4
STR	LR, [SP, #0]
; bufferSize end address is: 0 (R0)
; bufferSize start address is: 0 (R0)
;modbus.c,207 :: 		unsigned char i=0;
;modbus.c,208 :: 		for (i = 0; i < bufferSize; i++)UART2_Write(frame[i]);
; i start address is: 20 (R5)
MOVS	R5, #0
; bufferSize end address is: 0 (R0)
; i end address is: 20 (R5)
...
NOP
NOP
;modbus.c,210 :: 		previousTimeout = millis(); // initialize timeout delay
BL	_millis+0
...
_calculateCRC:
;modbus.c,213 :: 		unsigned int calculateCRC(unsigned char bufferSize)
; bufferSize start address is: 0 (R0)
; bufferSize end address is: 0 (R0)
; bufferSize start address is: 0 (R0)
;modbus.c,217 :: 		temp = 0xFFFF;
; temp start address is: 12 (R3)
MOVW	R3, #65535
;modbus.c,219 :: 		for (i = 0; i < bufferSize; i++)
; i start address is: 16 (R4)
MOVS	R4, #0
...
;modbus.c,221 :: 		temp = temp ^ frame[i];
MOVW	R1, #lo_addr(_frame+0)
MOVT	R1, #hi_addr(_frame+0)
ADDS	R1, R1, R4
LDRB	R1, [R1, #0]
...
;modbus.c,224 :: 		flag = temp & 0x0001;
AND	R2, R5, #1
UXTH	R2, R2
;modbus.c,225 :: 		temp >>= 1;
LSRS	R5, R5, #1
UXTH	R5, R5
;modbus.c,226 :: 		if (flag)  temp ^= 0xA001;
CMP	R2, #0
IT	EQ
...
L_calculateCRC23:
;modbus.c,231 :: 		temp2 = temp >> 8;
; temp start address is: 12 (R3)
LSRS	R2, R3, #8
UXTH	R2, R2
;modbus.c,232 :: 		temp = (temp << 8) | temp2;
LSLS	R1, R3, #8
UXTH	R1, R1
; temp end address is: 12 (R3)
ORR	R2, R1, R2, LSL #0
UXTH	R2, R2
;modbus.c,233 :: 		temp &= 0xFFFF;
MOVW	R1, #65535
AND	R1, R2, R1, LSL #0
;modbus.c,234 :: 		return temp; // the returned value is already swopped - crcLo byte is first & crcHi byte is last
UXTH	R0, R1
;modbus.c,235 :: 		}
L_end_calculateCRC:
BX	LR
